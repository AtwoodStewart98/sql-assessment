...
app.get("/api/user/:userId/vehiclecount", (req, res, next) => {
  const db = app.get("db");
  db
    .get_count_of_cars([req.params.userId])
    .then(response => res.json(response))
    .catch(err => console.log(err));
});
...
app.get("/api/user/:userId/vehicle", (req, res, next) => {
  const db = app.get("db");
  db
    .get_vehicles_by_id([req.params.userId])
    .then(response => {
...
app.put("/api/vehicle/:vehicleId/user/:userId", (req, res, next) => {
  const db = app.get("db");
  db
    .update_vehicle_owner([req.params.vehicleId, req.params.userId])
    .then(response => res.json(response))
    .catch(err => console.log(err));
});
...
app.delete("/api/user/:userId/vehicle/:vehicleId", (req, res, next) => {
  const db = app.get("db");
  db
    .delete_owner([req.params.userId, req.params.vehicleId])
    .then(response => res.json(response))
    .catch(err => console.log(err));
});
...
app.delete("/api/vehicle/:vehicleId", (req, res, next) => {
  const db = app.get("db");
  db
    .delete_vehicle_by_id([req.params.vehicleId])
    .then(response => res.json(response))
    .catch(err => console.log(err));
});
